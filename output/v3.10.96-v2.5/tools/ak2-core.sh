...
ui_print "$SDK found via ak2-core";
chmod -R 755 $bin;
...
write_boot() {
  ...
  if [ "$SDK" != 19 ]; then
    ui_print "installing zImage"
    kernel=/tmp/anykernel/kernel/zImage;
  else
    ui_print "installing zImage for legacy android"
    kernel=/tmp/anykernel/kernel/zImage_legacy;
  fi;

  ui_print "updating DTB";
  dtb="--dt /tmp/anykernel/kernel/dtb";
